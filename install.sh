...
# Crear directorios y copiar imágenes de la extensión
...
DESCRIPTION="\
    {${LF}\
        ...
        translationMap: {${LF}\
          'ja': {${LF}\
              'gui.extension.${EXTENSION_ID}blocks.name': 'Micro:bit Serial',${LF}\
              'gui.extension.${EXTENSION_ID}blocks.description': 'Conectar a Micro:bit y enviar datos vía Serial.'${LF}\
          },${LF}\
          'ja-Hira': {${LF}\
              'gui.extension.${EXTENSION_ID}blocks.name': 'Micro:bit Serial',${LF}\
              'gui.extension.${EXTENSION_ID}blocks.description': 'Conectar a Micro:bit y enviar datos vía Serial.'${LF}\
