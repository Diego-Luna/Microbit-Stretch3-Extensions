...
async function loadModelFromLinks(classNamesLink, modelJsonLink, modelBinLink) {
    const classNamesResponse = await fetch(classNamesLink);
    classNames = await classNamesResponse.json();

    model = await tf.loadLayersModel(tf.io.browserHTTPRequest(modelJsonLink));
    await model.loadWeights(tf.io.browserHTTPRequest(modelBinLink));
